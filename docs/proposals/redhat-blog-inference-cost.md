...
Kubernetes resource allocation has always had an efficiency problem. Teams size CPU and memory requests based on estimates, workloads over-provision, and idle capacity accumulates silently. The difference with GPUs is that the cost of getting this wrong is an order of magnitude higher — and the measurement tooling hasn't kept up.
...
OpenCost's existing Kubernetes cost allocation logic already handles CPU, RAM, and general GPU costs. What's missing is the inference layer: the ability to connect infrastructure costs to the token stream flowing through vLLM, and to expose that cost in terms that are meaningful for AI platform decisions.
...
**Allocation-based cost per model** counts everything attributed to running that model: the GPU memory reserved for its weights at all times, the compute consumed during active inference, and its share of shared infrastructure components like the EPP gateway and KV cache storage. This is the cost of *having the model available*. It reconciles to your infrastructure bill and answers: *"what is this model costing us?"*
...
| `llm_input_cost_per_million_tokens` | Cost per 1M input (prompt) tokens |
| `llm_output_cost_per_million_tokens` | Cost per 1M output (generation) tokens |

All metrics carry labels for `model_name`, `model_variant`, `namespace`, `cost_basis` (usage vs. allocation), and standard OpenCost attribution labels for team and workload chargeback.

Input and output token costs are separated because they reflect different compute workloads. Input tokens trigger the prefill phase; output tokens drive the decode phase. In disaggregated serving deployments — where prefill and decode run on separate hardware — this distinction is load-bearing for accurate cost attribution.  KV cache hits directly impact the cost of processing input tokens, which is of course taken into acccount in the per token metrics.
...
A finance team running chargeback can query costs by namespace and team label, generating showback reports for billing periods. A FinOps team can surface underutilized models and quantify the savings from right-sizing or decommissioning. A smart router — which llm-d is developing — can consume the MCP server API directly to factor per-token cost into routing decisions alongside latency and throughput metrics.

---

## Where things stand

A POC was implemented and tested on a cluster with 109 GPUs and 30 deployed AI models.

OpenCost has been updated to provide the core AI inference metrics and APIs, and is currently under review by the OpenCost community.




A working proof of concept demonstrates the core approach: inference cost metrics (total cost, cost per million tokens and input vs output token differentiation), multi-namespace support, Prometheus export and REST APIs. The POC validates that OpenCost's existing infrastructure is the right foundation — it already handles GPU allocation, Kubernetes integration, idle and shared costs and Prometheus pipelines. The inference cost layer adds the token-rate and compute-time logic that connects infrastructure costs to the token stream.

The roadmap from here covers on the OpenCost side wasted GPU costs (improving on OpenCost's current GPU idle detection for LLM-specific patterns), integration with the OpenCost UI, workload and team attribution, and optimization savings tracking — measuring the cost impact of KV cache hits, prefix caching, and routing decisions.  On the llm-d side, capturing workload and tenant metrics and deployment with OpenCost are work in progress.

Both projects are open source and contributions are welcome. **Update this with relevant links when ready to publish!!**

**Only include if accepted !!!
We'll be presenting this work at KubeCon 2026 in November in Salt Lake City — come find us if you want to talk through the design or bring your own cost visibility problem.**
